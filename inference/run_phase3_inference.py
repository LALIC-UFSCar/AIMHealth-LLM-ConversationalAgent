    ...
    if source == 'AMIVE':
        ...
        if affinities:
            ...
            for category, field in [
                ...
                ('Movies', 'Preferred Genres'),
                ('Games', 'Preferred Game Genres'),
            ]:
                values = affinities.get(category, {}).get(field, [])
                if isinstance(values, list):
                    values = ', '.join(str(value) for value in values[:2])
                if values:
                    top_interests.append(f"{category.lower()}: {values}")
                if len(top_interests) == 3:
                    break
            if top_interests:
                bullets.append(f"• Personal interests: {'; '.join(top_interests)}")
